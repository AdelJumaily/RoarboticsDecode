...
public class BlueBack extends LinearOpMode { //updated
    ...
    static final Vector2d path0 = new Vector2d(-45,-43); // Shooting position
    static final Vector2d path1 = new Vector2d(-12,-20); // Move to pick up ball part 1
    static final Vector2d path2 = new Vector2d(-12,-30); // Move to pick up balls part 2 (optional)
    static final Vector2d path3 = new Vector2d(-12,-50); // Move to pick up balls part 3
    static final Vector2d path4 = new Vector2d(-45,-43);  // Shooting position
    static final Vector2d path5 = new Vector2d(0,10); // Lever position part 1
            ...
            switch (currentState) {
                ...
                case WAIT0:
                    if (waitTimer.milliseconds() >= 2000) {
                        ...
                        currentState = BlueBack.State.MTSP;
                        waitTimer.reset();
                    }
...
//                    }
                    if (!drive.isBusy()) {
                        currentState = BlueBack.State.SHOOT1;
                        waitTimer.reset();
                    }
                    break;
                ...
                case SHOOT1:
                    ...
                    while (drive.robot.getDCShooterSubsystem().getStateMachine().getState() == DCShooterStateMachine.State.SHOOT) {
                        ...
                        if (waitTimer.milliseconds() >= 2000){
                            ...
                            waitTimer.reset();
                            drive.followTrajectorySequenceAsync(P1);
                            currentState = BlueBack.State.MTBLP;
                        }
                    }
                    break;
